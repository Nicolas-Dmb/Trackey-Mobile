...
function ModifCommonKey(){
    ...
    let PutKey = async() =>{
        ...
        let response = await fetch(`https://www.apitrackey.fr/api/CommonKey/${key.id}/`,{
            method:'PATCH',
            headers:{
    ...
    return(
        <View style={globalStyles.page}>
            ...
            <Text style={styles.textForm}>Numero du trousseau :</Text>
                <TextInput style={styles.textInput} placeholder='Nom' keyboardType='numeric' value={name.toString()} onChangeText={setName}/>
            ...
            <Text style={styles.textForm}>Photo des clés :</Text>
                ...
                {image ? (<Image source={{ uri: image }} style={{ width: tabBarHeight*0.2, height: tabBarHeight*0.2, margin:10 }}/>):(<Image source={{ uri: key.image }} style={{ width: tabBarHeight*0.2, height: tabBarHeight*0.2 }}/>)}
